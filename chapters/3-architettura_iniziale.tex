...
un Crontab secondo un intervallo regolare di 10 minuti. Questo script è a tutti gli
...
\begin{itemize}
  ...
  \item \textbf{log:} semplice tabella utilizzata per salvare messaggi di log
    ...
    log, la gravità, e il messaggio di log stesso. È utile per individuare
    problemi durante le esecuzioni di task direttamente dal frontend.
\end{itemize}
...
nei prossimi mesi, sono emersi immediatamente evidenti i problemi di scalabilità
che si sarebbero presentati se si fosse mantenuta l'architettura attuale.
...
sia per la configurazione iniziale, ma soprattutto per la gestione e
manutenzione futura del sistema.

Un altro problema, a cui porterebbe un simile approccio, è lo \textbf{spreco di
risorse}: dato che l'implementazione attuale non può sfruttare al meglio
